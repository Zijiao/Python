...
dayToHostPairTuple = access_logs.map(lambda log: (log.date_time.timetuple().tm_mday, log.host))

dayGroupedHosts = dayToHostPairTuple.groupByKey()


dayHostCount = dayGroupedHosts.map(lambda (day, hostList): (day, set(hostList)))


dailyHosts = (dayHostCount
              .map(lambda (day, hostSet): (day, len(hostSet)))
              .sortByKey()
              .cache())
...
daysWithHosts = dailyHosts.map(lambda (day, hostsCount): day).collect()
hosts = dailyHosts.map(lambda (day, hostsCount): hostsCount).collect()
...
dayAndHostTuple = access_logs.map(lambda log: (log.date_time.timetuple().tm_mday, log.host))
...
avgDailyReqPerHost = (sortedByDay
                       .map(lambda (day, hostList): (day, len(hostList)/len(set(hostList)))))
...
daysWithAvg = avgDailyReqPerHost.map(lambda (day, ave): day).collect()
avgs = avgDailyReqPerHost.map(lambda (day, ave): ave).collect()
...
badEndpointsTop20 = badEndpointsSum.sortBy(lambda pair: -1 * pair[1]).take(20)
...
errHostsTop25 = errHostsSum.sortBy(lambda pair: -1 * pair[1]).take(25)
...
errDateCountPairTuple = badRecords.map(lambda log: (log.date_time.timetuple().tm_mday, 1))
...
daysWithErrors404 = errDateSorted.map(lambda (day, err): day).collect()
errors404ByDay = errDateSorted.map(lambda (day, err): err).collect()
...
topErrDate = errDateSorted.takeOrdered(5, lambda (day, err): -1 * err)
...
hourCountPairTuple = badRecords.map(lambda log: (log.date_time.timetuple()[3], 1))
...
hoursWithErrors404 = hourRecordsSorted.map(lambda (hour, err): hour).collect()
errors404ByHours = hourRecordsSorted.map(lambda (hour, err): err).collect()
