...
def simpleTokenize(string):
    ...
    """
    return filter(lambda x: x != "", re.split(split_regex, string.lower()))
...
def tokenize(string):
    ...
    """
    return filter(lambda x: (x != "") and x not in stopwords, re.split(split_regex, string.lower()))
...
amazonRecToToken = (amazonSmall
                    ...
                    .map(lambda line: [line[0], tokenize(line[1])])
...
# RDD stucture [ID, [list of tokens]]
...
def countTokens(vendorRDD):
    ...
    """
    return sum(vendorRDD.map(lambda line: len(line[1])).collect())
...
def findBiggestRecord(vendorRDD):
    ...
    return (vendorRDD
           ...
           .takeOrdered(1, lambda x: -len(x[1]))
            )
...
def tf(tokens):
    ...
    total = 0
    for _, token in enumerate(token_dict):
        total += token_dict[token]
    total = float(total)
    
    for _, token in enumerate(token_dict):
        token_dict[token] = token_dict[token] / total
    return token_dict 
...
corpusRDD = (amazonRecToToken
             .map(lambda line: (line[0], line[1:])).union(googleRecToToken.map(lambda line: (line[0], line[1:]))) 
             )
...
def getUniqueTokens(tokenCountPairTuple):
    """
    return a RDD of unique (token, count) pairs
    """
    list_tokens = tokenCountPairTuple.collect()
    list_token = [token for sublist in list_tokens for token in sublist]
    return sc.parallelize(list_token)
...
def idfs(corpus):
    ...
    N = corpus.count()
    print corpus.take(1)
    uniqueTokens = corpus.map(lambda line: set(line[1]))
    tokenCountPairTuple = uniqueTokens.map(lambda line: [(token, 1) for token in line])
    uniqueTokensRDD = getUniqueTokens(tokenCountPairTuple)
    tokenSumPairTuple = uniqueTokensRDD.reduceByKey(lambda x, y: x + y)
    answer =  tokenSumPairTuple.map(lambda line: (line[0], N / float(line[1])))
    return answer
...
trueDupsCount = trueDupsRDD.filter(lambda record: record[1][1] == 'gold').count()
avgSimDups = sum(trueDupsRDD.map(lambda record: record[1][0]).collect()) / float(trueDupsCount)

nonDupsRDD = (sims
               .subtractByKey(goldStandard)
              )

avgSimNon = sum(nonDupsRDD.map(lambda record: record[1]).collect()) / float(nonDupsRDD.count())
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda line: [line[0], " ".join(line[1:])]).map(lambda line: [line[0], tokenize(line[1])])
googleFullRecToToken = google.map(lambda line: [line[0], " ".join(line[1:])]).map(lambda line: [line[0], tokenize(line[1])])
...
idfsFull = (idfs(amazonFullRecToToken.union(googleFullRecToToken)))
...
amazonWeightsRDD = tfidf(tokenize(string1), idfsFullBroadcast)
