...
#  a 'id' (a state could really be anything you want) and this 'id' is
...
class cMenu:
   ...
   #                       shown: (id, 'label', 'value', image) where text is the text
   ...
   def get_current_id(self):
      return self.menu_items[self.selection]['id']

   ## ---[ get_current_label ]--------------------------------------------------
   def get_current_label(self):
      return self.menu_items[self.selection]['label']

   ## ---[ get_current_value ]--------------------------------------------------
   def get_current_value(self):
      return self.menu_items[self.selection]['value']

   ## ---[ get_current_image ]--------------------------------------------------
   def get_current_image(self):
      return self.menu_items[self.selection]['b_image']

   ## ---[ get_button_formatted_display ]---------------------------------------
   def get_button_formatted_display(self, button):
      if button['value']:
   ...
   def set_current_value(self, new_value):
      self.menu_items[self.selection]['value'] = new_value
      # If this button is not an image, set the width and height based on the
      # text
      if self.menu_items[self.selection]['b_image'] == None:
         width, height = self.font.size(self.get_button_formatted_display(self.menu_items[self.selection]))
         button_rect = pygame.Rect((0, 0), (width, height))
      # Else this button is a graphic button, so create the width and height
      # based on the image provided
      else:
         width, height = self.menu_items[self.selection]['b_image'].get_size()
      ...
      self.menu_items[self.selection]['rect'] = button_rect
   ...
   def set_font(self, font):
      ...
      for button in self.menu_items:
         if button['b_image'] == None:
            width, height = self.font.size(button['label'])
            button['rect'][2] = width
   ...
   def create_button(self, button_info):
      ...
      new_button = {'id'      : button_info[0],
                    ...
                    'offset'  : (0, 0),
                    'redraw'  : set_redraw,
                    'b_image' : button_info[3],  # base image
      ...
      # If this button is not an image, set the width and height based on the
      # text
      if new_button['b_image'] == None:
         width, height = self.font.size(self.get_button_formatted_display(new_button))
         button_rect = pygame.Rect((0, 0), (width, height))
      # Else this button is a graphic button, so create the width and height
      # based on the image provided
      else:
         width, height = new_button['b_image'].get_size()
         offset = (self.image_highlight_offset, self.image_highlight_offset)
         new_width  = width  + 2 * offset[0]  # Make room for the highlight on
         new_height = height + 2 * offset[1]  # all sides
         button_rect = pygame.Rect((0, 0), (new_width, new_height))
   ...
   def set_button_images(self):
      for button in self.menu_items:
         ...
         if button['b_image'] == None:
            ...
            text_image   = r(self.get_button_formatted_display(button), True, self.s_color)
            ...
            text_image   = r(self.get_button_formatted_display(button), True, self.u_color)
   ...
   #
   def update(self, e, redraw = False):
      ...
      # safe guard the selection index
