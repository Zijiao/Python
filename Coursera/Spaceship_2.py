...
class Sprite:
    ...
    def collide(self, other_object):
        [x1, x2] = self.get_pos()
        [y1, y2] = other_object.get_pos()
        r1 = self.get_radius()
        r2 = other_object.get_radius()
        if math.sqrt((x1 - y1) ** 2 + (x2 - y2) ** 2) <= r1+r2:
            return True
        else:
            return False
...
def draw(canvas):
    ...
    # determine if the ship hit any of the rocks (step 2.3)
    if group_collide(rock_group, my_ship):
        lives += -1
    
    # detect missile/rock collisions (step 4.2)
...
def rock_spawner():
    global rock_group
    if len(rock_group) <= 12 and started and started:
