...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    averageRating = sum(IDandRatingsTuple[1]) / float(count)
    return (IDandRatingsTuple[0], (count, averageRating))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda record: (record[1][1][1], record[1][0], record[1][1][0]))
                             )
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda record: record[2] > 500)
...
def computeError(predictedRDD, actualRDD):
    ...
    predictedReformattedRDD = predictedRDD.map(lambda record: ((record[0], record[1]), record[2]))

    # Transform actualRDD into the tuples of the form ((UserID, MovieID), Rating)
    actualReformattedRDD = actualRDD.map(lambda record: ((record[0], record[1]), record[2]))

    # Compute the squared error for each matching entry (i.e., the same (User ID, Movie ID) in each
    # RDD) in the reformatted RDDs using RDD transformtions - do not use collect()
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD)
                        .map(lambda record: (record[1][0] - record[1][1]) ** 2)
                       )
...
validationForPredictRDD = validationRDD.map(lambda record: (record[0], record[1]))
...
myModel = ALS.train(trainingRDD, bestRank, seed=seed, iterations=iterations,
                      lambda_=regularizationParameter)
testForPredictingRDD = testRDD.map(lambda record: (record[0], record[1]))
...
trainingAvgRating = (trainingRDD.map(lambda line: line[2]).reduce(lambda x, y: x + y) 
                       / trainingRDD.map(lambda line: line[2]).count())
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda line: (line[0], line[1], trainingAvgRating))
...
# TODO: Replace <FILL IN> with appropriate code
trainingWithMyRatingsRDD = sc.parallelize(myRatedMovies).union(trainingRDD)
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda line: line[0] not in set([pair[1] for pair in myRatedMovies]))
                      .map(lambda line: (0, line[0]))
                     )
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda line: (line[0], line[1][0]))

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda line: (line[1], line[2]))
