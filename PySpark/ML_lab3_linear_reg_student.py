...
# Zijiao: excellent visualization!
...
minYear = onlyLabels.takeOrdered(1, lambda x: x)[0]
maxYear = onlyLabels.takeOrdered(1, lambda x: -x)[0]
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return math.sqrt(labelsAndPreds
              .map(lambda line: squaredError(line[0], line[1]))
              .reduce(lambda x, y: x + y)
              / float(labelsAndPreds.count())
                    )
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPredsTrain = parsedTrainData.map(lambda line: (line.label, averageTrainYear))
rmseTrainBase = calcRMSE(labelsAndPredsTrain)

labelsAndPredsVal = parsedValData.map(lambda line: (line.label, averageTrainYear))
rmseValBase = calcRMSE(labelsAndPredsVal)

labelsAndPredsTest = parsedTestData.map(lambda line: (line.label, averageTrainYear))
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        labelsAndPredsTrain = trainData.map(lambda line: getLabeledPrediction(w, line))
        errorTrain[i] = calcRMSE(labelsAndPredsTrain)

        # Calculate the `gradient`.  Make use of the `gradientSummand` function you wrote in (3a).
        # Note that `gradient` sould be a `DenseVector` of length `d`.
        gradient = trainData.map(lambda line: gradientSummand(w, line)).reduce(lambda x, y: x + y)
...
labelsAndPreds = parsedValData.map(lambda line: getLabeledPrediction(weightsLR0, line))
...
def twoWayInteractions(lp):
    ...
    newFeatures = [ x * y for (x, y) in pairs]
    allFeatures = np.hstack([lp.features, newFeatures])
    return LabeledPoint(lp.label, allFeatures)
